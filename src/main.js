...
const openNewModalWindow = async (url) => {
...
const done = async () => {
	...
	if (input.value.startsWith('/view-logs')) {
		await openNewModalWindow(`${CLIENT_URL}/view-logs?username=${input.value.split(' ')[1]}`);
	}
	else {
		await openNewModalWindow(`${CLIENT_URL}/command?command=${input.value}`);
	}
